if UndeadGuardsAI == nil then
    _G.UndeadGuardsAI = class({})
end

function UndeadGuardsAI:Start()
	...
	Timers:CreateTimer(function()
		...
		for table_number,guard in pairs(undeadMeleeGuards) do
			if IsValidEntity(guard) then
				local spawn_position = Entities:FindByName(nil, ("undead_melee_guard_"..table_number)):GetAbsOrigin()
				...
				if math.ceil(guard_position:Length()) ~= math.ceil(spawn_position:Length()) then
					...
					--UndeadGuardsAI:GodsStrength( guard )
					--UndeadGuardsAI:ShieldAttack( guard )
					UndeadGuardsAI:ReturnToSpawnPosition( guard , spawn_position)
		...
		for table_number,guard in pairs(undeadRangedGuards) do
			if IsValidEntity(guard) then
				--UndeadGuardsAI:Assassinate( guard )
			end
		end
...
function UndeadGuardsAI:GodsStrength( guard )
...
function UndeadGuardsAI:ShieldAttack( guard )
...
function UndeadGuardsAI:ReturnToSpawnPosition( guard, spawn_position )
...
function UndeadGuardsAI:Assassinate( guard )
	local ability = guard:GetAbilityByIndex(0)
	local guard_mana = guard:GetMana()
